...
export class Nft {
  ...
  public async all(
    ...
    const payload = this._program.registry
      .createType('(String, String)', ['Nft', 'All'])
      .toHex();
  ...
  public async canDelete(
    ...
    const payload = this._program.registry
      .createType('(String, String)', ['Nft', 'CanDelete'])
      .toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public async collectionOwner(
    ...
    const payload = this._program.registry
      .createType('(String, String)', ['Nft', 'CollectionOwner'])
      .toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public async config(
    ...
    const payload = this._program.registry
      .createType('(String, String)', ['Nft', 'Config'])
      .toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public async description(
    ...
    const payload = this._program.registry
      .createType('(String, String)', ['Nft', 'Description'])
      .toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public async getPaymentForMint(
    ...
    const payload = this._program.registry
      .createType('(String, String)', ['Nft', 'GetPaymentForMint'])
      .toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public async imgLinksAndData(
    ...
    const payload = this._program.registry
      .createType('(String, String)', ['Nft', 'ImgLinksAndData'])
      .toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public async marketplaceAddress(
    ...
    const payload = this._program.registry
      .createType('(String, String)', ['Nft', 'MarketplaceAddress'])
      .toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public async name(
    ...
    const payload = this._program.registry
      .createType('(String, String)', ['Nft', 'Name'])
      .toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public async nonce(
    ...
    const payload = this._program.registry
      .createType('(String, String)', ['Nft', 'Nonce'])
      .toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public async permissionToMint(
    ...
    const payload = this._program.registry
      .createType('(String, String)', ['Nft', 'PermissionToMint'])
      .toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public async totalNumberOfTokens(
    ...
    const payload = this._program.registry
      .createType('(String, String)', ['Nft', 'TotalNumberOfTokens'])
      .toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public subscribeToTransferredEvent(
    ...
    return this._program.api.gearEvents.subscribeToGearEvent(
      ...
      ({ data: { message } }) => {
        ...
        if (
          getServiceNamePrefix(payload) === 'Nft' &&
          getFnNamePrefix(payload) === 'Transferred'
        ) {
          callback(
            this._program.registry
              .createType(
      ...
      ({ data: { message } }) => {
        ...
        if (
          ...
          callback(
            this._program.registry
              .createType(
                '(String, String, {"token_id":"u64","nft_data":"NftData"})',
                message.payload,
              )[2]
              .toJSON() as unknown as {
              token_id: number | string | bigint;
  ...
  public subscribeToApprovedEvent(
    ...
    return this._program.api.gearEvents.subscribeToGearEvent(
      ...
      ({ data: { message } }) => {
        ...
        if (
          getServiceNamePrefix(payload) === 'Nft' &&
          getFnNamePrefix(payload) === 'Approved'
        ) {
          callback(
            this._program.registry
              .createType(
                '(String, String, {"to":"[u8;32]","token_id":"u64"})',
                message.payload,
              )[2]
              .toJSON() as unknown as {
              to: ActorId;
  ...
  public subscribeToApprovalRevokedEvent(
    ...
    return this._program.api.gearEvents.subscribeToGearEvent(
      ...
      ({ data: { message } }) => {
        ...
        if (
          getServiceNamePrefix(payload) === 'Nft' &&
          getFnNamePrefix(payload) === 'ApprovalRevoked'
        ) {
          callback(
            this._program.registry
              .createType(
                '(String, String, {"token_id":"u64"})',
                message.payload,
              )[2]
  ...
  public subscribeToExpandedEvent(
    ...
    return this._program.api.gearEvents.subscribeToGearEvent(
      ...
      ({ data: { message } }) => {
        ...
        if (
          getServiceNamePrefix(payload) === 'Nft' &&
          getFnNamePrefix(payload) === 'Expanded'
        ) {
          callback(
            this._program.registry
              .createType(
  ...
  public subscribeToConfigChangedEvent(
    ...
    return this._program.api.gearEvents.subscribeToGearEvent(
      ...
      ({ data: { message } }) => {
        ...
        if (
          getServiceNamePrefix(payload) === 'Nft' &&
          getFnNamePrefix(payload) === 'ConfigChanged'
        ) {
          callback(
            this._program.registry
              .createType(
  ...
  public subscribeToUsersForMintAddedEvent(
    ...
    return this._program.api.gearEvents.subscribeToGearEvent(
      ...
      ({ data: { message } }) => {
        ...
        if (
          getServiceNamePrefix(payload) === 'Nft' &&
          getFnNamePrefix(payload) === 'UsersForMintAdded'
        ) {
          callback(
            this._program.registry
              .createType(
  ...
  public subscribeToUserForMintDeletedEvent(
    ...
    return this._program.api.gearEvents.subscribeToGearEvent(
      ...
      ({ data: { message } }) => {
        ...
        if (
          getServiceNamePrefix(payload) === 'Nft' &&
          getFnNamePrefix(payload) === 'UserForMintDeleted'
        ) {
          callback(
            this._program.registry
              .createType(
  ...
  public subscribeToLiftRestrictionMintEvent(
    ...
    return this._program.api.gearEvents.subscribeToGearEvent(
      ...
      ({ data: { message } }) => {
        ...
        if (
          getServiceNamePrefix(payload) === 'Nft' &&
          getFnNamePrefix(payload) === 'LiftRestrictionMint'
        ) {
          callback(null);
        }
      },
  ...
  public subscribeToMetadataAddedEvent(
    ...
    return this._program.api.gearEvents.subscribeToGearEvent(
      ...
      ({ data: { message } }) => {
        ...
        if (
          getServiceNamePrefix(payload) === 'Nft' &&
          getFnNamePrefix(payload) === 'MetadataAdded'
        ) {
          callback(
            this._program.registry
              .createType(
                '(String, String, {"nft_id":"u64","metadata":"String"})',
                message.payload,
              )[2]
              .toJSON() as unknown as {
              nft_id: number | string | bigint;
      ...
      ({ data: { message } }) => {
        ...
        if (
          ...
          callback(
            this._program.registry
              .createType(
                '(String, String, {"nft_id":"u64","img_link":"String"})',
                message.payload,
              )[2]
              .toJSON() as unknown as {
              nft_id: number | string | bigint;
  ...
  public subscribeToMetadataChangedEvent(
    ...
    return this._program.api.gearEvents.subscribeToGearEvent(
      ...
      ({ data: { message } }) => {
        ...
        if (
          getServiceNamePrefix(payload) === 'Nft' &&
          getFnNamePrefix(payload) === 'MetadataChanged'
        ) {
          callback(
            this._program.registry
              .createType(
                '(String, String, {"nft_id":"u64","metadata":"Vec<String>"})',
                message.payload,
              )[2]
              .toJSON() as unknown as {
              nft_id: number | string | bigint;
  ...
  public subscribeToMetadataDeletedEvent(
    ...
    return this._program.api.gearEvents.subscribeToGearEvent(
      ...
      ({ data: { message } }) => {
        ...
        if (
          getServiceNamePrefix(payload) === 'Nft' &&
          getFnNamePrefix(payload) === 'MetadataDeleted'
        ) {
          callback(
            this._program.registry
              .createType(
                '(String, String, {"nft_id":"u64"})',
                message.payload,
              )[2]
