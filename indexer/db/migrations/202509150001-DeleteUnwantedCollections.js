module.exports = class DeleteUnwantedCollections202509150001 {
    ...
    async up(db) {
        ...
        await db.query(
            `DELETE FROM "bid" WHERE auction_id IN (
                SELECT id FROM "auction" WHERE nft_id IN (
        ...
        await db.query(
            `DELETE FROM "auction" WHERE nft_id IN (
                SELECT id FROM "nft" WHERE collection_id = ANY($1)
            )`,
            [ids]
        )
        ...
        await db.query(
            `DELETE FROM "offer" WHERE nft_id IN (
                SELECT id FROM "nft" WHERE collection_id = ANY($1)
            )`,
            [ids]
        )
        ...
        await db.query(
            `DELETE FROM "sale" WHERE nft_id IN (
                SELECT id FROM "nft" WHERE collection_id = ANY($1)
            )`,
            [ids]
        )
        ...
        await db.query(
            `DELETE FROM "transfer" WHERE nft_id IN (
                SELECT id FROM "nft" WHERE collection_id = ANY($1)
            )`,
            [ids]
        )
        ...
        await db.query(
            `DELETE FROM "nft" WHERE collection_id = ANY($1)`,
            [ids]
        )
