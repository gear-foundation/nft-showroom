...
export class NftShowroom {
  ...
  public mint(collection_address: ActorId, img_link_id: number | string | bigint | null): TransactionBuilder<null> {
    if (!this._program.programId) throw new Error('Program ID is not set');
    return new TransactionBuilder<null>(
      this._program.api,
      this._program.registry,
  ...
  public subscribeToConfigUpdatedEvent(
    callback: (data: { config: Config; minimum_value_for_trade: number | string | bigint }) => void | Promise<void>,
  ): Promise<() => void> {
    return this._program.api.gearEvents.subscribeToGearEvent('UserMessageSent', ({ data: { message } }) => {
      if (!message.source.eq(this._program.programId) || !message.destination.eq(ZERO_ADDRESS)) {
        return;
