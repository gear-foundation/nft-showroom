...
type GetPriceSchema = {
  (minValue: string, isInclusive?: boolean): GetPriceSchemaReturn;
...
function usePriceSchema() {
  ...
  const getPriceSchema: GetPriceSchema = (_minValue?: string, isInclusive?: boolean) => {
    ...
    const minValueMessage = isInclusive
    ...
    return z
      ...
      .refine((value) => (isInclusive ? value.gt(minValue) : value.gte(minValue)), minValueMessage)
