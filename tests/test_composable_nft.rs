...
use composable_nft_io::{ComposableNftInit, ComposableNftAction, Config, StateQuery as StateQueryNft, StateReply as StateReplyNft, ComposableNftEvent };
...
fn successful_basics() {
    ...
    let res = nft_collection.send(USERS[1], ComposableNftAction::Mint{combination: vec![0,0,0]});
