...
use nft_io::{NftInit, Config as ConfigNft, StateQuery as StateQueryNft, StateReply as StateReplyNft};
...
fn create_success() {
    ...
    // Update config
    let res = update_config(&marketplace, ADMINS[0], Some(200_000_000_000), Some(7_200_000));
    assert!(!res.main_failed());
...
fn create_failures() {
    ...
    assert!(res.main_failed());
    let res = update_config(&marketplace, USERS[0], Some(200_000_000_000), Some(7_200_000));
    assert!(res.main_failed());
...
fn sale_success() {
    ...
    let res = nft_collection.send(USERS[1], nft_io::NftAction::Approve { to: addres_marketplace.into(), token_id: 0 });
    assert!(!res.main_failed());
    ...
    let state_reply = marketplace.read_state(StateQuery::All).expect("Unexpected invalid state.");
